...
def get_dataset():
    ...
    if NN_info.dataset == 1:
        ...
        features, labels = make_circles(n_samples = 100, noise = 0.1, random_state=42)
    elif NN_info.dataset == 4:
        name = ("Random")
        from sklearn.datasets import make_classification
        features, labels = make_classification(n_samples=100, n_features=2, 
                           n_informative=2, n_redundant=0, 
                           n_clusters_per_class=2, random_state=42)
    elif NN_info.dataset == 3:
        name = ("Blobs")
        from sklearn.datasets import make_blobs
        features, labels = make_blobs(n_samples=100, centers=2, n_features=2, random_state=42)
